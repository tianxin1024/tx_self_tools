...
OperatorDesc CreateOpDesc() {
  // define operator
  std::vector<int64_t> inputShape0{900, 512};
  std::vector<int64_t> inputShape1{512, 80};
  std::vector<int64_t> outputShape{900, 80};
  std::string opType = "MatrixMulCust";
  aclDataType dataType = ACL_FLOAT32;
  aclFormat format = ACL_FORMAT_ND;
  OperatorDesc opDesc(opType);
  opDesc.AddInputTensorDesc(dataType, inputShape0.size(), inputShape0.data(),
                            format);
...
bool SetInputData(OpRunner &runner) {
  for (size_t i = 0; i < runner.NumInputs(); ++i) {
    ...
    bool ret = ReadFile(filePath, fileSize, runner.GetInputBuffer<void>(i),
                        runner.GetInputSize(i));
    if (!ret) {
...
bool ProcessOutputData(OpRunner &runner) {
  for (size_t i = 0; i < runner.NumOutputs(); ++i) {
    ...
    if (!WriteFile(filePath, runner.GetOutputBuffer<void>(i),
                   runner.GetOutputSize(i))) {
      ERROR_LOG("Write output[%zu] failed.", i);
      return false;
    }

    INFO_LOG("Write output[%zu] success. output file = %s", i,
             filePath.c_str());
  }
  return true;
}

bool RunMatrixMulOp(bool isDevice) {
  // Create op desc
