...
static inline void micro_kernel_8x8_packB(
    const float* __restrict A, int K,
    const float* __restrict Bp, // Kx8
    float* __restrict C, int N, // C 行主序，N 为总列数
    float alpha)
{
    // 对应每行两组累加器：前4列、后4列
    ...
    // k 维度按4展开：一次载入4个 k 切片，每个切片包含 8 列（分两段 4）
    int k = 0;
    for (; k + 3 < K; k += 4) {
        // B 的四个切片（每个切片两段：前4/后4列）
    ...
    for (int r = 0; r < 8; ++r) {
        ...
        float32x4_t sL = accL[r], sR = accR[r];
        // alpha
        sL = vmulq_n_f32(sL, alpha);
        sR = vmulq_n_f32(sR, alpha);
        vst1q_f32(crow + 0, sL);
        vst1q_f32(crow + 4, sR);
    }
}
                  ...
                  float alpha)
{
    ...
    for (int j = 0; j + 7 < N; j += 8) {
        ...
        for (; i + 7 < M; i += 8) {
            ...
            micro_kernel_8x8_packB(Abase, K, Bpanel.data(), Cbase, N, alpha);
        }
