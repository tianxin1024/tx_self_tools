    ...
    int M, int N, int K, float alpha)
{
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            ...
            C[i*N + j] = alpha * sum;   // <<< 乘以 alpha
        }
    }
}

// ===================== Optimized NEON (带 alpha) =====================
    ...
    int M, int N, int K, int i, int j, float alpha)
{
    ...
    for (int r = 0; r < 8; ++r) {
        for (int c = 0; c < 4; ++c) {
            ...
            C_AT(C, M, N, i + r, j + c) = alpha * sum; // <<< 乘以 alpha
        }
    }
}
    ...
    int M, int N, int K, int i0, int i1, int j0, int j1, float alpha)
{
    for (int i = i0; i < i1; ++i) {
        for (int j = j0; j < j1; ++j) {
            ...
            C_AT(C, M, N, i, j) = alpha * sum; // <<< 乘以 alpha
        }
    }
}
    ...
    float alpha)
{
    ...
    for (; j + nr - 1 < N; j += nr) {
        int i = 0;
        for (; i + mr - 1 < M; i += mr) {
            micro_kernel_8x4_f32(A, B, C, M, N, K, i, j, alpha);
        }
        if (i < M) {
            kernel_scalar_f32(A, B, C, M, N, K, i, M, j, j + nr, alpha);
        }
    }
}
...
int main() {
    ...
    gemm_baseline(A.data(), B.data(), C1.data(), M, N, K, alpha);
    auto t1 = std::chrono::high_resolution_clock::now();
    double baseline_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();

    // neon
    auto t2 = std::chrono::high_resolution_clock::now();
    gemm_at_bt_f32_neon(A.data(), B.data(), C2.data(), M, N, K, alpha);
